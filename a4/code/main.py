...
if __name__ == "__main__":
    ...
    if question == "2":
        ...
        # One-vs-all
        model = LogisticRegression(multi_class='ovr', fit_intercept=False, C=500)
        model.fit(XMultiValid, yMultiValid)
        print("\nTraining error %.3f" % utils.classification_error(model.predict(XMulti), yMulti))
        print("Validation error %.3f" % utils.classification_error(model.predict(XMultiValid), yMultiValid))
        print("# nonZeros: %d" % (model.coef_ != 0).sum())

        # Softmax
        model = LogisticRegression(multi_class='multinomial', solver='lbfgs', fit_intercept=False, C=500)
        model.fit(XMultiValid, yMultiValid)
        print("\nTraining error %.3f" % utils.classification_error(model.predict(XMulti), yMulti))
        print("Validation error %.3f" % utils.classification_error(model.predict(XMultiValid), yMultiValid))
        print("# nonZeros: %d" % (model.coef_ != 0).sum())
